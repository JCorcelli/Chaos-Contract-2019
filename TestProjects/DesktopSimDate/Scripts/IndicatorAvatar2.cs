...
namespace NPCSystem
{
	...
	public class IndicatorAvatar2	: AbstractButtonHandler {
		...
		protected float constraintRadius = 0f;
		protected float originDistance = 0f;
		...
		protected virtual void Start() {
			...
			constraintRadius = distanceLimiter.radius;
			
			gravity = Physics.gravity;
			
			target = CameraHolder.instance.targetB; // maybe later it's something else
		...
		public void OnFall () {
			...
			rigidbody.position = targetA.position;
		...
		protected void ApplyPhysicalMovement(){
			...
			// moving far from origin, or moving close enough
			originDistance = Vector3.Distance(targetA.position, transform.position);
			
			
			if (constraintRadius - originDistance <= breakingDistance || distance < breakingDistance ) // stopping distance essentially
			{
