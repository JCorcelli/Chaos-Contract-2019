...
namespace NPCSystem
{
	...
	public class IndicatorAvatar	: AbstractButtonHandler {
		...
		protected void ApplyPhysicalMovement(){
			...
			if (distance < breakingDistance) // stopping distance essentially
			{
...
#if UNITY_EDITOR
			// in scene view
			Debug.DrawLine(transform.position + (transform.up * 0.2f), transform.position + (-transform.up * m_GroundCheckDistance));
#endif
			// 0.1f is a small offset to start the ray from inside the character
			// it is also good to note that the transform position in the sample assets is at the base of the character
			
			if (Physics.Raycast(transform.position + (transform.up * 0.2f), -transform.up, out hitInfo, m_GroundCheckDistance, activeLayers))
			{
				touchingGround = true;
