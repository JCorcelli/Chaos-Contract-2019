...
{
	public class MagnetBubble : UpdateBehaviour {
		/*
		...
		*/
		...
		protected override void OnLateUpdate () {
			...
			if (ncanvas != canvas || currentScreenHeight != Screen.height)
			{
				// something changed!
				canvas = ncanvas;
		...
		protected void CompassInViewReverse(){
			...
			goal = Camera.main.WorldToScreenPoint(spriteTransform.position + camTransform.forward * (Vector3.Distance(camTransform.position, spriteTransform.position) + 1f)) + offset * imageTransform.localScale.y;
		...
		protected void RepositionCompass(){
			goal = Camera.main.WorldToScreenPoint(spriteTransform.position) + offset * imageTransform.localScale.y ;
		...
		protected float GetRad(){
			return imageTransform.sizeDelta.y / 2f - 5f;
		}
		protected void SetColliderRadius(){
			float rad = imageTransform.sizeDelta.y / 2f - 5f;
			
			offset = Vector3.up * rad;
			collider.radius = rad;
		}
			...
			*/
			...
			if (!Cross(r, s).IsZero() && (0 <=u && u <=1) && (0 <=t && t <= 1))
			{
				...
				if (visibleBehind)
					pos.z = 0;
				//imageTransform.position = pos;
				goal = pos; // in this case the side of screen is the exact location
				return true;
			}
