...
namespace TestProject.Cameras
{
	public class ZoomToFitFrom : MonoBehaviour {
		...
		protected Vector3 outPosition = Vector3.one; // reused for storing a position between methods
		...
		void Update () {
			...
			if (FitAll(new Vector3[]{targetA.position, targetB.position}))
			{
		...
		public Vector3 Zoom(float multiplier = 1f) {
		...
		public Vector3 Fit(Vector3 point) {
			...
			float radHFOV = Mathf.Atan(Mathf.Tan(radAngle / 2) * cam.aspect);
