...
namespace NPCSystem
{
	...
	public class ConstrainTransform : UpdateBehaviour {
		...
		protected void StayInZone() {
			...
			if ( d   > scaledDistanceLimiter  )
			{
				...
				backmove = Vector3.ProjectOnPlane(backmove, m_GroundNormal);
				transform.position = backmove;
			}
		...
		protected void CheckGroundStatus()
		{
			...
			if (Physics.Raycast(transform.position + (transform.up * 0.1f), -transform.up, out hitInfo, 1f, activeLayers))
			{
				touchingGround = true;
