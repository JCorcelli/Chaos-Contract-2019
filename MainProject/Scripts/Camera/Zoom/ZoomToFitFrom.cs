...
{
	public class ZoomToFitFrom : MonoBehaviour {
		...
		public bool fitToViewHeight = true;
		public bool FitAll(Vector3 [] vList, Vector3 facing) {
			Vector3[] newV = new Vector3[vList.Length];
			int i = 0;
			foreach (Vector3 v in vList)
			{
				// here, I need to check if it's above camera, if it is I FitHeight
				
				bool outsideY = false;
				if (fitToViewHeight)
				{
					Vector3 onViewPoint = cam.WorldToViewportPoint(v);
					outsideY = onViewPoint.y < 0f || onViewPoint.y > 1f;
				}
				...
				if (outsideY)
				{
					...
					newV[i] = (Fit(v)); // maybe have "Fit" do the check.
				i ++;
			}
		...
		*/
		public Vector3 Fit(Vector3 point) {
			...
			float radWFOV = Mathf.Atan(Mathf.Tan(radAngle * 0.5f) * cam.aspect);
			
			
			float opposite = Vector3.Distance(point, frontOrtho);
			float adjacent = opposite/radWFOV ;
		...
		public Vector3 FitHeight(Vector3 point) {
			...
			// *** Camera angle FOV ***
			float radAngle = cam.fieldOfView * Mathf.Deg2Rad;
			float radHFOV = Mathf.Atan(Mathf.Tan(radAngle * 0.5f));
