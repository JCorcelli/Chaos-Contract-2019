...
var m_AgentWidth: float = 1f;
private var wray : Ray;
...
function FindChild(waypInst : Waypoint)
{
	...
	for (var node : Waypoint in waypointList)
	{
		if(node.ID != waypInst.ID)
		{
			...
			Dir.Normalize();
			wray = new Ray(waypInst.Pos, Dir);
			if((Physics.SphereCast(wray, m_AgentWidth, Vector3.Distance(waypInst.Pos, node.Pos), layerMask)) == false)
			{
...
// Find path // start : Vector3, dest : Vector3
function FindPath() 
{

	var start = Startpoint.localPosition ;
	var dest = Endpoint.localPosition ;
	...
	wray = new Ray(start, Dir);
	if((Physics.SphereCast(wray, m_AgentWidth, Vector3.Distance(start, dest), layerMask)) == false)
	{
		// Add start location
		pathList.Push(start);
		...
		while(opendList.length != 0)
		{
			...
			for (var child : Waypoint in closedList[closedList.length-1].childList)
			{
				...
				if(skip == 0)
				{
					...
					Dir = dest - child.Pos;
					Dir.Normalize();
					wray = Ray(child.Pos, Dir);
					if((Physics.SphereCast(wray, m_AgentWidth, Vector3.Distance(child.Pos, dest), layerMask)) == false)
					{
	...
	gameObject.SendMessage ("SuccessPath", resultPath); // path is successful
	if (resultPath)
	{
		gameObject.SendMessage("ClearPath");
		for (i  = 0; i < pathList.length; i++) 
		{
			gameObject.SendMessage ("SetPath", pathList[i]); // the list
		}
	}
